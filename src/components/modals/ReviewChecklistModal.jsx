...
//     if (!title || !loanType || !assignedToRM) {
...
//         customerName,
//         customerNumber,
//         documents: [], // Start empty or preload if you want
//       };

//       const result = await createChecklist(checklistPayload).unwrap();
//       message.success(`Checklist created with ID: ${result._id}`);
//       // Optionally reset form or navigate
//     } catch (error) {
//       console.error("Error creating checklist:", error);
//       message.error(error?.data?.error || "Failed to create checklist.");
//     }
//   };
...
//       </Select>
//       <Input
//         placeholder="Assign to RM (user ID)"
//         value={assignedToRM}
...
//         style={{ marginBottom: 12 }}
//       />
//       <Button type="primary" loading={isLoading} onClick={handleSubmit} block>
//         Create Checklist
//       </Button>
//     </div>
//   );
...
const ReviewChecklistModal = ({ checklist, open, onClose }) => {
  ...
  const handleAddNewDocument = () => {
    ...
    setDocs((prevDocs) => {
      const newDocument = {
        ...
        name: newDocName.trim(),
        category: selectedCategoryName,
        status: "pending from Rm",
        action: "pending from Rm",
        comment: "",
        fileUrl: null,
      };
      return [...prevDocs, newDocument];
  ...
  /** ==============================
   *  SUBMIT TO CHECKER  (DCL No)
   * ============================== */
  const submitToCheckers = async () => {
    if (!checklist?.dclNo)
      return message.error("DCL No missing.");
...

    {
      ...
      render: (text, record) => (
        ...
        >
          ...
          <Option value="Pending_from_rm">Pending from RM</Option>
          ...
          <Button
            ...
            onClick={() => {
              try {
                ...
                if (!newWindow) {
                  message.error("Popup blocked! Please allow popups to view the file.");
                }
              } catch (error) {
                console.error(error);
      ...
      >
        {checklist && (
          <>
            ...
            >
              <Descriptions size="middle" column={{ xs: 1, sm: 2, lg: 3 }}>
                <Descriptions.Item label="DCL No">{checklist.dclNo}</Descriptions.Item> {/* ✅ UPDATED */}
                <Descriptions.Item label="Created At">{checklist.createdAt}</Descriptions.Item>
                <Descriptions.Item label="Loan Type">{checklist.loanType}</Descriptions.Item>
                <Descriptions.Item label="Created By">{checklist.createdBy?.name}</Descriptions.Item>
                <Descriptions.Item label="RM">{checklist.assignedToRM?.name}</Descriptions.Item>
                <Descriptions.Item label="Co-Checker">{checklist.assignedToCoChecker?.name || "Pending"}</Descriptions.Item>
